...
# Data structures, 10p
# There should be an easier way, but I can't figure it out
#
# Use SortedList with with binary search
# TC: O(nlogn + mlogn)
...
mx_prices = list(map(int, input().split()))

tickets = SortedList(tickets)
for p in mx_prices:
    idx = tickets.bisect_right(p) - 1
    if idx >= 0:
        print(tickets.pop(idx))
    else:
        print(-1)
