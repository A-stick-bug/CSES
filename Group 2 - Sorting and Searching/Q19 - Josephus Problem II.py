...
# Simple math + data structures, 10p
# Without relying on SortedList, it would be 12p
#
# We no longer have the 'cut in half' guarantee from part 1
# Instead, we simulate with data structures
# We use SortedList's O(logn) k-th element indexing, which is not supported by C++ multiset
# - If we wanted to use this method in C++, we need the built-in PBDS or use Fenwick/segment trees
#
# TC: O(nlogn)
...
arr = SortedList(range(n))  # 0-index

idx = k % len(arr)
removed = []
while arr:
    removed.append(arr.pop(idx))
    if arr:
        idx = (idx + k) % len(arr)  # note that all indices after idx got shifted left due to the removal

print(" ".join(map(lambda x: str(x + 1), removed)))  # convert to 1-index
