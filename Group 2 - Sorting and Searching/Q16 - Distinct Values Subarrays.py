...
#
# The idea is to sum the answers over all possible right endpoints
# Take a look at [G2 Q13 - Playlist](https://cses.fi/problemset/task/1141) first, the solution is almost identical
#
# TC: O(nlogn)
...
total = 0
cur = SortedList()  # current subarray
l = 0
for r in range(n):
    while arr[r] in cur:  # shrink left to maintain uniqueness
        cur.remove(arr[l])
        l += 1
    cur.add(arr[r])

    # sketch of correctness proof:
    # - the current range [l,r] maximize the size of the unique subarray ending at `r`
    # - we know that any subarray [i,r] is unique for i in [l,r], we have r-l+1 choices for i
    total += r - l + 1
